...
class Auth extends \wind\Auth
{
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        // 必须将结果集转换为数组
        // 如果使用mysql函数，使用orderRaw，否则使用order
        $ruleList = model('AuthRule')->cache('menulist')->where('ismenu', 1)->orderRaw('if(weigh=0,9999,weigh)')->select()->toArray();
        ...
        $single_multile = config('mySet.single_multile');
        $menu = Tree::instance()->getTreeMenu(0, '<li class="@class"><a href="@url" ' . ($single_multile == 'single' ? 'target="main_iframe"' : '') . ' addtabs="@id" url="@url" py="@py" pinyin="@pinyin"><i class="@icon"></i> <span>@title</span> <span class="pull-right-container">@caret @badge</span></a> @childlist</li>', $select_id, '', 'ul', 'class="treeview-menu"');
        return $menu;
    }
