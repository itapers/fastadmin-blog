...
class Extractor
{
    ...
    private static function consolidateAnnotations($method, $class)
    {
        ...
        if (empty($methodAnnotations) || isset($methodAnnotations['ApiInternal']) || $methodName == 'initialize' || $methodName == '_empty') {
            return [];
        }
        ...
        if (!isset($methodAnnotations['method'])) {
            $methodAnnotations['method'] = ['get'];
        }
        if (!isset($methodAnnotations['desc'])) {
            $methodAnnotations['desc'] = [$methodTitle];
        }
        ...
        if (!isset($methodAnnotations['title'])) {
            $methodAnnotations['title'] = [$methodTitle];
        }
        if (!isset($methodAnnotations['route'])) {
            $urlArr = [];
            $className = $class->getName();

            list($prefix, $suffix) = explode('\\' . Config::get('app.url_controller_layer') . '\\', $className);
            $prefixArr = explode('\\', $prefix);
            $suffixArr = explode('\\', $suffix);
            if ($prefixArr[0] == Config::get('app.app_name')) {
                $prefixArr[0] = '';
            }
            ...
            $methodAnnotations['route'] = [implode('/', $urlArr)];
        }
        ...
        $methodAnnotations['PermissionLogin'] = [!in_array('*', $noNeedLogin) && !in_array($methodName, $noNeedLogin)];
        $methodAnnotations['PermissionRight'] = [!in_array('*', $noNeedRight) && !in_array($methodName, $noNeedRight)];
