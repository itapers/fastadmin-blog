...
class Backend extends Controller
{
    ...
    public function initialize()
    {
        ...
        if (in_array($actionname, $this->noNeedRight)) {
            $this->auth->getRuleList(session('admin.id'));
        }
        // 非选项卡时重定向
        if (!$this->request->isPost() && !IS_AJAX && !IS_ADDTABS && !IS_DIALOG && 'addtabs' == input('ref')) {
        ...
        $config = [
            'site' => array_intersect_key($site, array_flip(['name', 'indexurl', 'cdnurl', 'version', 'timezone'])),
            'upload' => $upload,
        ...
        $config = array_merge($config, Config::pull("view_replace_str"));
        ...
        //渲染站点title
        $this->assign('title', config('site.name'));
        //当前窗口模式为单页模式还是多标签模式
        $this->assign('single_multile', config('mySet.single_multile'));
    }
