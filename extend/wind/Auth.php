...
class Auth
{
    ...
    public function getRuleList($uid)
    {
        ...
        //读取用户组所有权限规则
        $this->rules = Db::name($this->config['auth_rule'])->cache('userAuthRule')->where($where)->field('id,pid,condition,icon,name,title,ismenu')->select();
